...
<head>
    ...
    <title>{{$old_product->product->name}}</title>
    ...
    <link rel="stylesheet" href="{{ asset('home/css/chitietsanphamcu.css?version=1.2') }}">
...
<body>
    ...
    <section>
        ...
        <div id="Modal" class="modal" style="overflow: hidden;">
            <div class="modal-content" style="margin: 10% auto !important;">
                ...
                <form id="Form" action="{{url('/order_old_product',$old_product->id)}}" method="post">
                    @csrf
                    ...
                    <div class="div_modal_center">
                        ...
                        <div class="content">
                            <div class="div_design">
                                ...
                                <input class="text_color" type="text" style="width: 65%;" id="user_name" name="user_name" value="{{$user_name}}">
                            </div>

                            <div class="div_design">
                                <label style="width: 27%;"><b>Số điện thoại:</b></label>
                                <input class="text_color" type="text" style="width: 65%;" id="phone" name="phone" value="{{$user_phone}}">
                            </div>
        ...
        <div class="breadcrumb">
            <p><a href="/"><b>Trang chủ </b></a>><a href="{{url('dien-thoai-cu')}}"><b> Điện thoại cũ </b></a>><a href="{{url('dien-thoai-cu-gia-re',$old_product->product->slug)}}"><b style="color: blue;"> Điện thoại {{$old_product->product->name}} cũ</b></a></p>
        </div>
        ...
        <div class="chitietSanpham" style="margin-bottom: 10px; display: flex; gap: 5px; flex-wrap: wrap;">
            <div>
                <h1><b>Điện thoại {{$old_product->product->name}} cũ</b>
            </div>
            <div class="old_product_color"><b>Màu: {{$old_product->color->color}}</b></div>
        ...
        <div class="rowdetail group border_product">
            <div class="picture">
                ...
                <img src="{{ asset('old_product_img/' . $old_product->image1) }}" id="mainImage" style="object-fit: contain;">
                ...
                <div class="area_picture_small">
                    <div class="area_picture_small_container">
                        <div class="area_picture_small_item">
                            <img src="{{ asset('old_product_img/' . $old_product->image1) }}" onclick="changeImage('{{ $old_product->image1 }}')">
                        </div>
                        <div class="area_picture_small_item">
                            <img src="{{ asset('old_product_img/' . $old_product->image2) }}" onclick="changeImage('{{ $old_product->image2 }}')">
                        </div>
                        <div class="area_picture_small_item">
                            <img src="{{ asset('old_product_img/' . $old_product->image3) }}" onclick="changeImage('{{ $old_product->image3 }}')">
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="price_sale">
                <div class="area_price"><strong>{{ number_format($old_product->new_price, 0, ',', '.') }}₫</strong>

                    <b class="price_old">
                        {{ number_format($old_product->product->price, 0, ',', '.') }}₫
                    </b>
                    <?php
                ...
                <div class="area_info">
                    <div class="info">
                        ...
                        <div>
                            <p><i class="fa fa-money" aria-hidden="true"></i><b> Giá sản phẩm mới: </b>{{ number_format($old_product->product->price, 0, ',', '.') }}₫</p>
                        </div>
                        <?php
                        $discountPrice = $old_product->product->price - $old_product->new_price;
                        ?>
                        <div>
                            <p><i class="fa fa-usd" aria-hidden="true"></i><b> Tiết kiệm: </b>{{ number_format($discountPrice, 0, ',', '.') }}₫</p>
                        </div>
                        <div>
                            <p><i class="fa fa-shopping-bag" aria-hidden="true"></i><b> Sản phẩm đang có tại cửa hàng: </b></p>
                            <a href="{{url('cua-hang/' .$old_product->branch->slug)}}" target="_blank"> {{$old_product->branch->address}}, {{$old_product->branch->district}}, {{$old_product->branch->city}}</a>
                        </div>
                        <div>
                            <p><i class="fa fa-hand-o-right" aria-hidden="true"></i><a href="{{url('chi-tiet-san-pham/' .$old_product->product->slug)}}" target="_blank"> Xem sản phẩm mới</a></p>
                        </div>
                    </div>
                </div>
                <div class="area_info">
                    ...
                    <div class="info">
                        ...
                        <div>
                            <p>Thời hạn bảo hành: <b>{{$old_product->time_guarantee}} chính hãng</b></p>
                        </div>
                        <div>
                        ...
                        <div>
                            <p>Tình trạng sản phẩm: <b>{{$old_product->status_product}}</b></p>
                        </div>
                        <div>
                            <p>IMEI: <b>{{ substr($old_product->imei, 0, 4) . '***' . substr($old_product->imei, -4) }}</b></p>
                        </div>
                    ...
                    @if($old_product->status_sale==1)
                        ...
                        <div class="area_order">
                            @if (Route::has('login'))
                                ...
                                @else
                                <a class="buy_now" id="Button" href="{{url('login')}}">
                                    <b><i class="fa fa-cart-plus"></i> Đặt giữ hàng</b><br>
                                    <p> Tại siêu thị tối đa 24h</p>
                                </a>
            ...
            <div class="info_product">
                ...
                <ul class="info">
                    <li>
                        <p><b>Màn hình</b></p>
                        <div>{{$old_product->product->screen}}</div>
                    </li>
                    <li>
                        <p><b>Hệ điều hành</b></p>
                        <div>{{$old_product->product->software}}</div>
                    </li>
                    <li>
                        <p><b>Camera sau</b></p>
                        <div>{{$old_product->product->camera_sau}}</div>
                    </li>
                    <li>
                        <p><b>Camera trước</b></p>
                        <div>{{$old_product->product->camera_truoc}}</div>
                    </li>
                    <li>
                        <p><b>CPU</b></p>
                        <div>{{$old_product->product->chip}}</div>
                    </li>
                    <li>
                        <p><b>RAM</b></p>
                        <div>{{$old_product->product->ram}}</div>
                    </li>
                    <li>
                        <p><b>ROM</b></p>
                        <div>{{$old_product->product->rom}}</div>
                    </li>
                    <li>
                        <p><b>Cổng sạc</b></p>
                        <div>{{$old_product->product->port}}</div>
                    </li>
                    <li>
                        <p><b>SIM</b></p>
                        <div>{{$old_product->product->sim}}</div>
                    </li>
                    <li>
                        <p><b>PIN</b></p>
                        <div>{{$old_product->product->pin}} mAh</div>
                    </li>
                </ul>
            </div>
        </div>
        ...
        <div class="chitietSanpham" style="margin-bottom: 20px">
            ...
            <div class="rowdetail_youtube group">
                <div class="youtube">
                    <p>{{$old_product->product->description}}</p>
                    <br>
                    <iframe width="780px" height="500px" src="https://www.youtube.com/embed/{{$old_product->product->link}}" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" allowfullscreen></iframe>
                </div>
            </div>
    ...
    <script>
        ...
        function changeImage(imagePath) {
            ...
            var selectedItem = document.querySelector(`.area_picture_small_item img[src="{{ asset('old_product_img/') }}/${imagePath}"]`);
            selectedItem.closest('.area_picture_small_item').classList.add('selected');

            document.getElementById('mainImage').src = "{{ asset('old_product_img/') }}" + "/" + imagePath;
        ...
        function changeImageByArrow(direction) {
            ...
            var nextItem = document.querySelector(`.area_picture_small_item img[src="{{ asset('old_product_img/') }}/${nextImagePath}"]`);
            nextItem.closest('.area_picture_small_item').classList.add('selected');

            document.getElementById('mainImage').src = "{{ asset('old_product_img/') }}" + "/" + nextImagePath;
        }
