...
<body>
    ...
    <section>
        ...
        <div class="filterName">
            <h1 style="font-size: 25px;"><b>Danh sách sản phẩm có từ khóa "{{$searchText}}"</b></h1>
            <h1 style="font-size: 15px;"><b>Tìm thấy {{$count_result}} kết quả</b></h1>
        </div> <!-- End FilterName -->
        ...
        <div class="pagination">
            ...
            <a href="{{ $products->previousPageUrl() }}{{ $products->total() > 1 ? '&search=' . $searchText : '' }}">
                <i class="fa fa-angle-left"></i>
            </a>
            @endif

            @for ($i = 1; $i <= $products->lastPage(); $i++)
                <a href="{{ $products->url($i) }}{{ $products->total() > 1 ? '&search=' . $searchText : '' }}" class="{{ $i == $products->currentPage() ? 'current' : '' }}">
                    {{ $i }}
                </a>
                @endfor

                @if ($products->nextPageUrl())
                <a href="{{ $products->nextPageUrl() }}{{ $products->total() > 1 ? '&search=' . $searchText : '' }}">
